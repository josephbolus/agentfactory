...
Agent Factory supports macOS and Linux. The server, worker, and local launcher are
...
github_issue_poll_interval = "30s"
```
...
The server uses the GitHub CLI on its host to poll opted-in managed repositories
for open `needs-agent` issues. The setting controls that poll interval; repository
opt-in remains in Infrastructure.

The control-plane database contains prompts, results, and repository metadata.
Agent Factory creates the database, its marker, and SQLite WAL and shared-memory files
with owner-only permissions (`0600`). When a valid existing database is opened,
Agent Factory corrects broader permissions before use and rejects non-regular files or
...
or other users. Agent Factory validates the configured path before creating missing
...
Agent Factory reports each one as ready, missing, unauthenticated, or unhealthy, and
the Worker stays available while Git and at least one configured runtime are
ready.

Agent Factory migrates existing SQLite databases to the expanded worker capacity
range when the control plane starts.

With this `~/.factory/worker.toml` filename, Agent Factory defaults durable worker
state to `~/.factory/workers/worker`. The config filename, rather than `name`,
selects the state directory.

No worker repository list is required. Agent Factory detects local GitHub access with
...
non-bare Git checkout with an `origin` remote. Agent Factory starts legacy work from
...
the required GitHub access, and can acquire managed repositories. Agent Factory
...
Agent Factory stores state below `~/.factory` by default. Common overrides:
...
1. Stop the control plane and all workers. Keep the old Agent Factory data home
   unchanged so rollback remains possible.
2. Install the backed-up configuration into a new mode-`0700` Agent Factory data
   home. Update absolute paths only when the recovery host differs.
3. Restore with the same Agent Factory version that made the backup, or a newer
   compatible version:
...
3. Start the current Agent Factory control plane and open **Automations → Migrate
   legacy poller**.
...
5. Review every resolved source path, managed-repository identity and ID,
   ...
   `poller.toml`, Agent Factory shows that ledger-only queue and blocks Import. Restore
   the matching queue entry, stop the poller, and run Preview again so no
   observation identity is silently omitted.
6. Choose **Import disabled Automations**. Agent Factory verifies the exact Preview
...
8. Choose **Finalize and archive**. Agent Factory locks and verifies the same snapshot,
...
restarting Agent Factory rediscovers the active imported migration and preserves
...
- return to the migration dialog and run a new Preview against the stable
  source. Agent Factory does not partially import or archive a changed snapshot.

A worktree is retained

Agent Factory keeps worktrees when they are dirty or may contain unpublished work.
