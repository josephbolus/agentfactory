...
result after the container exits. It is intentionally separate from Agent Factory's
Worker protocol.

One reusable Job handles every execution. A run freezes an approved internal GitHub
...
version is pinned on the Job. Agent Factory does not receive or store the secret.
...
## Deploy to the Agent Factory Google Cloud project

The project name is `Agent Factory` and its project ID is `factory-505220`. Commands
...
The first proof supports approved internal GitHub HTTPS repositories only. `execute.sh`
...
- One internal repository and one Pi process per execution.
- No Agent Factory Work, Attempt, lease, or cancellation integration yet.
- No private repository authentication.
- No branch push or pull request publishing.
- No untrusted repository isolation.
- No automatic infrastructure provisioning from the Agent Factory UI.
