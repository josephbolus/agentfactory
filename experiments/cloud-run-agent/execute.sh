...
readonly repository_url="${REPOSITORY_URL:-https://github.com/josephbolus/agentfactory.git}"
...
if [[ ! "$repository_url" =~ ^https://github\.com/[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+\.git$ ]]; then
    printf 'REPOSITORY_URL must be a approved internal GitHub HTTPS clone URL\n' >&2
    exit 2
fi
