...
type Store struct {
	...
	githubIssues        githubIssueSource
}
...
func openStore(ctx context.Context, path string, existingOnly bool) (*Store, error) {
	...
	store := &Store{
		...
		defaultBuildRuntime: protocol.RuntimeCodex, githubIssues: githubCLI{},
	}
...
func scanManagedRepository(row scanner) (protocol.ManagedRepository, error) {
	...
	var enabled, issueIntakeEnabled int
	...
	repository.IssueIntakeEnabled = issueIntakeEnabled != 0
...
func (s *Store) setManagedRepositoryIssueIntake(
	ctx context.Context,
	repositoryID string,
	enabled bool,
) (protocol.ManagedRepository, error) {
	...
	result, err := s.db.ExecContext(ctx, `
		UPDATE repositories
		SET issue_intake_enabled = ?, updated_at = ?
		WHERE id = ? AND centrally_managed = 1
	`, enabled, now, repositoryID)
	if err != nil {
		return protocol.ManagedRepository{}, unavailable(err)
	}
