...
func normalizeTask(input protocol.SaveTaskRequest, now time.Time) (normalizedTask, error) {
	...
	if value.workflowID != "" {
		workflowID, err := protocol.NormalizeRepositoryWorkflowID(value.workflowID)
		if err != nil {
			return value, invalid("invalid_workflow_id", err.Error())
		}
...
func (s *Store) CreateTask(ctx context.Context, input protocol.SaveTaskRequest) (protocol.Task, error) {
	...
	if err := validateTaskWorkflow(ctx, tx, value.repositoryIDs, value.workflowID); err != nil {
		return protocol.Task{}, err
	}
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO tasks(
			...
			concurrency_limit, execution_profile_id, outcome_contract, pipeline_id, repository_workflow_id, generation, archived, migration_only, schedule_enabled,
	...
	`, id, value.name, value.nameKey, value.prompt, value.runtime, value.timeoutSeconds,
		value.concurrencyLimit, nullableString(value.executionProfileID), value.outcomeContract, value.pipelineID, value.workflowID, value.scheduleEnabled, nullableString(value.cron), nullableString(value.timezone),
...
func (s *Store) UpdateTask(ctx context.Context, id string, input protocol.SaveTaskRequest) (protocol.Task, error) {
	...
	var currentPipelineID, currentWorkflowID string
	var pendingDue, scheduleRetry sql.NullInt64
	var scheduleHealthStatus, scheduleHealthCode string
	err = tx.QueryRowContext(ctx, `
		SELECT archived, migration_only, read_only, outcome_contract, COALESCE(pipeline_id, ?), COALESCE(repository_workflow_id, ''), pending_due_at, schedule_retry_at,
		       schedule_health_status, schedule_health_code
		FROM tasks WHERE id = ?
	`, protocol.DefaultPipelineID, id).Scan(&archived, &migrationOnly, &readOnly, &currentOutcome, &currentPipelineID, &currentWorkflowID, &pendingDue, &scheduleRetry,
	...
	if strings.TrimSpace(input.WorkflowID) == "" {
		value.workflowID = currentWorkflowID
	}
	if err := validateTaskWorkflow(ctx, tx, value.repositoryIDs, value.workflowID); err != nil {
		return protocol.Task{}, err
	}
	...
	result, err := tx.ExecContext(ctx, `
		UPDATE tasks SET
			...
			concurrency_limit = ?, execution_profile_id = ?, outcome_contract = ?, pipeline_id = ?, repository_workflow_id = ?, generation = generation + 1, schedule_enabled = ?,
	...
	`, value.name, value.nameKey, value.prompt, value.runtime, value.timeoutSeconds,
		value.concurrencyLimit, nullableString(value.executionProfileID), value.outcomeContract, value.pipelineID, value.workflowID, value.scheduleEnabled, value.scheduleEnabled, nullableString(value.cron),
...
func validateTaskWorkflow(ctx context.Context, tx *sql.Tx, repositoryIDs []string, workflowID string) error {
	...
	for _, repositoryID := range repositoryIDs {
		...
		if err := tx.QueryRowContext(ctx, `
			SELECT EXISTS(
				SELECT 1 FROM repository_workflow_catalogs catalog
				JOIN repository_workflow_entries entry ON entry.repository_id = catalog.repository_id
				WHERE catalog.repository_id = ? AND catalog.status = 'healthy' AND entry.workflow_id = ?
			)
		`, repositoryID, workflowID).Scan(&exists); err != nil {
			return unavailable(err)
		}
		if exists == 0 {
...
func (s *Store) Task(ctx context.Context, id string) (protocol.Task, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT task.id, task.name, task.prompt, task.runtime, COALESCE(task.execution_profile_id, ''), COALESCE(task.repository_workflow_id, ''), task.timeout_seconds, task.concurrency_limit,
	...
	`, protocol.DefaultPipelineID, protocol.DefaultPipelineID, id).Scan(&task.ID, &task.Name, &task.Prompt, &task.Runtime, &task.ExecutionProfileID, &task.WorkflowID,
...
func taskWorkflowSnapshot(
	workflows map[string]protocol.RepositoryWorkflowSnapshot,
	repositoryID, workflowID string,
) (protocol.RepositoryWorkflowSnapshot, error) {
	workflow, exists := workflows[repositoryID]
	if !exists || workflow.ID != workflowID {
		return protocol.RepositoryWorkflowSnapshot{}, conflict(
			"workflow_not_found",
			fmt.Sprintf("workflow %s is not available for this repository", workflowID),
		)
	}
	return workflow, nil
}
...
func (s *Store) admitTask(
	...
	if frozen != nil {
		...
		err := tx.QueryRowContext(ctx, `
			SELECT id, name, prompt, runtime, COALESCE(execution_profile_id, ''),
			       COALESCE(repository_workflow_id, ''), timeout_seconds, concurrency_limit,
		...
		`, protocol.DefaultPipelineID, taskID).Scan(&snapshot.ID, &snapshot.Name, &snapshot.Prompt, &snapshot.Runtime,
			&snapshot.ExecutionProfileID, &snapshot.WorkflowID, &snapshot.TimeoutSeconds, &snapshot.ConcurrencyLimit, &snapshot.Generation,
	...
	if snapshot.WorkflowID != "" {
		if len(snapshot.RepositoryWorkflows) == 0 {
			if err := validateTaskWorkflow(ctx, tx, taskRepositoryIDs(snapshot.Repositories), snapshot.WorkflowID); err != nil {
				return protocol.RunDetail{}, false, err
			}
			snapshot.RepositoryWorkflows = make(map[string]protocol.RepositoryWorkflowSnapshot, len(snapshot.Repositories))
			for _, repository := range snapshot.Repositories {
				workflow, err := loadWorkflowSnapshot(ctx, tx, repository.ID, snapshot.WorkflowID)
				if err != nil {
					return protocol.RunDetail{}, false, err
				}
				snapshot.RepositoryWorkflows[repository.ID] = workflow
				if snapshot.Workflow.ID == "" {
					snapshot.Workflow = workflow
				}
			}
		} else {
			workflow, err := taskWorkflowSnapshot(snapshot.RepositoryWorkflows, snapshot.Repositories[0].ID, snapshot.WorkflowID)
			if err != nil {
				return protocol.RunDetail{}, false, err
			}
			snapshot.Workflow = workflow
	...
	for position, repository := range snapshot.Repositories {
		...
		if snapshot.WorkflowID != "" {
			workflow, err = taskWorkflowSnapshot(snapshot.RepositoryWorkflows, repository.ID, snapshot.WorkflowID)
			if err != nil {
				return protocol.RunDetail{}, false, err
			}
		}
		targetPrompt := resolveTaskWorkflowPrompt(resolvedPrompt, workflow, snapshot.WorkflowID != "")
		resolvedStages, err := resolveSessionStages(snapshot, targetPrompt, runID, target)
		if err != nil {
			return protocol.RunDetail{}, false, err
		}
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO sessions(
				...
				repository_workflow_snapshot
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, sessionID, runID, repository.ID, repository.RemoteIdentity, targetPrompt, snapshot.Runtime,
			...
			boundedUTF8Bytes(blockedReason, protocol.MaxWaitingReasonBytes), mustJSON(workflow)); err != nil {
			return protocol.RunDetail{}, false, unavailable(err)
		}
...
func (s *Store) runListEntry(ctx context.Context, id string) (protocol.Run, error) {
	...
	for repositoryID, workflow := range run.Task.RepositoryWorkflows {
		workflow.Instructions = ""
		run.Task.RepositoryWorkflows[repositoryID] = workflow
	}
...
func (s *Store) Run(ctx context.Context, id string) (protocol.RunDetail, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		SELECT session.id, session.run_id, session.repository_id, session.repository_identity,
		       session.resolved_prompt, session.repository_workflow_snapshot, session.required_runtime, session.timeout_seconds,
		       ...
		       session.source_key, session.source_reference, session.source_title,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&session.ID, &session.RunID, &session.RepositoryID, &session.RepositoryIdentity,
			&session.ResolvedPrompt, &workflowSnapshot, &session.RequiredRuntime, &session.TimeoutSeconds,
			...
			&session.Target.SourceTitle, &session.Target.ContextSnapshot,
			&session.Target.PublishBranch, &session.PredecessorWorkID,
		...
		if len(workflowSnapshot) != 0 && string(workflowSnapshot) != "{}" {
			if err := json.Unmarshal(workflowSnapshot, &session.Workflow); err != nil {
				rows.Close()
				return detail, unavailable(err)
			}
		}
