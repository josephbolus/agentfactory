...
func TestBuildAdmissionSnapshotsSelectedRepositoryWorkflow(t *testing.T) {
	store := newTestStore(t)
	...
	store.githubIssues = &fakeWorkflowSource{commit: "commit-1", files: map[string][]byte{
		...
		".factory/workflows/qa.md":        []byte("---\nid: qa\ntitle: QA regression\n---\nRun the QA regression suite."),
	}}
	if _, err := store.RefreshRepositoryWorkflowCatalog(context.Background(), repository.ID); err != nil {
		t.Fatal(err)
	}
	admission, err := store.AdmitBuild(context.Background(), protocol.BuildRequest{
		RequestKey: "workflow-build", Workflow: "qa", WorkflowSpecified: true,
		References: []string{"https://github.com/acme/api/issues/7"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if admission.Run.Run.Task.WorkflowID != "qa" ||
		!strings.Contains(admission.Run.Sessions[0].ResolvedPrompt, "Run the QA regression suite.") ||
		admission.Run.Sessions[0].Target.SourceTitle != "" {
		t.Fatalf("workflow admission = %#v", admission.Run)
	}
	var snapshot string
	if err := store.db.QueryRow(`SELECT repository_workflow_snapshot FROM sessions WHERE id = ?`, admission.Run.Sessions[0].ID).Scan(&snapshot); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(snapshot, `"id":"qa"`) || !strings.Contains(snapshot, `"commit_sha":"commit-1"`) {
		t.Fatalf("workflow snapshot = %s", snapshot)
	}
}

func TestDefaultBuildRefreshesWorkflowAndRebuildsLatestRevision(t *testing.T) {
	store := newTestStore(t)
	repository, _, err := store.CreateManagedRepository(context.Background(), protocol.CreateManagedRepositoryRequest{
		RemoteIdentity: "github.com/acme/api",
	})
	...
	source := &fakeWorkflowSource{
		commit: "commit-1",
		files:  map[string][]byte{".factory/workflows/implement.md": []byte("first implementation")},
	}
	store.githubIssues = source
	request := protocol.BuildRequest{
		RequestKey: "default-build", References: []string{"https://github.com/acme/api/issues/9"},
	}
	initial, err := store.AdmitBuild(context.Background(), request)
	if err != nil {
		t.Fatal(err)
	}
	if source.workflowCalls != 1 || !strings.Contains(initial.Run.Sessions[0].ResolvedPrompt, "first implementation") {
		t.Fatalf("initial workflow calls = %d, run = %#v", source.workflowCalls, initial.Run.Sessions[0])
	}
	...
	source.workflowErr = errors.New("workflow source unavailable")
	replayed, err := store.AdmitBuild(context.Background(), request)
	if err != nil || replayed.Result != protocol.AdmissionReplayed || source.workflowCalls != 1 {
		t.Fatalf("replay = %#v, calls = %d, error = %v", replayed, source.workflowCalls, err)
	}

	source.workflowErr = nil
	source.commit = "commit-2"
	source.files = map[string][]byte{".factory/workflows/implement.md": []byte("second implementation")}
	rebuilt, err := store.AdmitBuild(context.Background(), protocol.BuildRequest{
		RequestKey: "default-build-rebuild", References: request.References, Rebuild: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	if source.workflowCalls != 2 || !strings.Contains(rebuilt.Run.Sessions[0].ResolvedPrompt, "second implementation") ||
		strings.Contains(rebuilt.Run.Sessions[0].ResolvedPrompt, "first implementation") {
		t.Fatalf("rebuilt workflow calls = %d, run = %#v", source.workflowCalls, rebuilt.Run.Sessions[0])
	}
	if rebuilt.Run.Sessions[0].PredecessorWorkID != initial.Run.Sessions[0].ID {
		t.Fatalf("rebuild predecessor = %q, want %q", rebuilt.Run.Sessions[0].PredecessorWorkID, initial.Run.Sessions[0].ID)
	}
	if repository.ID == "" {
