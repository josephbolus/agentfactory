...
func (s *Store) AdmitBuild(ctx context.Context, input protocol.BuildRequest) (protocol.BuildAdmission, error) {
	return s.admitBuild(ctx, input, protocol.OutcomeAgentUpdate, nil, nil)
}

func (s *Store) admitGitHubIssueBuild(ctx context.Context, input protocol.BuildRequest, title string) (protocol.BuildAdmission, error) {
	return s.admitBuild(ctx, input, protocol.OutcomeProcessExit, map[string]string{input.References[0]: title}, nil)
}

func (s *Store) admitGitHubIssueBuildWithWorkflow(
	ctx context.Context,
	input protocol.BuildRequest,
	title, repositoryID string,
	workflow protocol.RepositoryWorkflow,
) (protocol.BuildAdmission, error) {
	return s.admitBuild(ctx, input, protocol.OutcomeProcessExit, map[string]string{input.References[0]: title}, map[string]protocol.RepositoryWorkflowSnapshot{
		repositoryID: workflow.Snapshot(),
	})
}

func (s *Store) admitBuild(
	ctx context.Context,
	input protocol.BuildRequest,
	outcomeContract protocol.OutcomeContract,
	sourceTitles map[string]string,
	workflowSnapshots map[string]protocol.RepositoryWorkflowSnapshot,
) (protocol.BuildAdmission, error) {
	...
	// Replay intentionally wins before any workflow refresh or other reads.
	admission, replayed, err := s.replayBuild(ctx, canonical.RequestKey, fingerprint)
	if err != nil {
	...
	if len(workflowSnapshots) == 0 {
		if _, supportsCatalog := s.githubIssues.(githubWorkflowSource); supportsCatalog {
			if err := s.refreshBuildWorkflows(ctx, normalized); err != nil {
				return protocol.BuildAdmission{}, err
			}
		}
	}
	...
	for index := range targets {
		targets[index].sourceTitle = sourceTitles[targets[index].reference.Reference]
		if len([]byte(targets[index].sourceTitle)) > protocol.MaxSourceTitleBytes {
			return protocol.BuildAdmission{}, invalid(
				"invalid_source_title",
				fmt.Sprintf("source title exceeds %d bytes", protocol.MaxSourceTitleBytes),
			)
		}
	}
	...
	snapshot := protocol.TaskSnapshot{
		...
		WorkflowID:         protocol.RepositoryWorkflowFallback,
	}
	...
	workflowID := normalized.Workflow
	if workflowID == "" {
		workflowID = protocol.RepositoryWorkflowFallback
	}
	if len(targets) > 0 {
		workflow, err := buildWorkflowSnapshot(ctx, tx, targets[0].repository.ID, workflowID, workflowSnapshots)
		if err != nil {
			return protocol.BuildAdmission{}, err
		}
		snapshot.WorkflowID, snapshot.Workflow = workflow.ID, workflow
	}
	...
	`, runID, canonical.RequestKey, fingerprint, protocol.StandardBuildProcedureID,
		snapshotJSON, executionJSON, outcomeContract, now, now); err != nil {
		return protocol.BuildAdmission{}, unavailable(err)
	}
	...
	for position, target := range targets {
		...
		frozen := protocol.WorkTarget{
			...
			SourceReference: target.reference.Reference, SourceTitle: target.sourceTitle,
			PublishBranch: workPublishBranch(workID),
		}
		workflow, err := buildWorkflowSnapshot(ctx, tx, target.repository.ID, workflowID, workflowSnapshots)
		if err != nil {
			return protocol.BuildAdmission{}, err
		}
		resolvedPrompt := resolveStandardBuildPrompt(frozen, workflow)
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO sessions(
				...
				source_reference, source_title, context_snapshot, publish_branch, predecessor_work_id,
				execution_owner, waiting_reason, repository_workflow_snapshot
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, workID, runID, target.repository.ID, target.repository.RemoteIdentity, resolvedPrompt, runtime,
			...
			frozen.SourceReference, frozen.SourceTitle, frozen.ContextSnapshot, frozen.PublishBranch,
...
func buildWorkflowSnapshot(
	...
	selected map[string]protocol.RepositoryWorkflowSnapshot,
) (protocol.RepositoryWorkflowSnapshot, error) {
	if workflow, ok := selected[repositoryID]; ok {
		if workflow.ID != workflowID {
			return protocol.RepositoryWorkflowSnapshot{}, conflict(
				"workflow_selection_conflict",
				"the selected repository workflow does not match the Build request",
			)
		}
		return workflow, nil
	}
	return loadWorkflowSnapshot(ctx, tx, repositoryID, workflowID)
}

func (s *Store) replayBuild(ctx context.Context, requestKey string, fingerprint []byte) (protocol.BuildAdmission, bool, error) {
	var runID string
	var existingFingerprint []byte
	err := s.db.QueryRowContext(ctx, `
		SELECT id, request_digest FROM runs WHERE request_key = ?
	`, requestKey).Scan(&runID, &existingFingerprint)
	if errors.Is(err, sql.ErrNoRows) {
		return protocol.BuildAdmission{}, false, nil
	}
	if err != nil {
		return protocol.BuildAdmission{}, false, unavailable(err)
	}
	if !bytes.Equal(existingFingerprint, fingerprint) {
		return protocol.BuildAdmission{}, false, conflict(
			"request_key_conflict",
			"request_key was already used with different Build inputs",
		)
	}
	detail, err := s.Run(ctx, runID)
	if err != nil {
		return protocol.BuildAdmission{}, false, err
	}
	return protocol.BuildAdmission{
		Result: protocol.AdmissionReplayed, RequestKey: requestKey, Run: detail,
	}, true, nil
}

func (s *Store) refreshBuildWorkflows(ctx context.Context, input protocol.NormalizedBuildInput) error {
	_, ok := s.githubIssues.(githubWorkflowSource)
	if !ok {
		return conflict("workflow_catalog_unavailable", "repository workflow catalogs require a GitHub workflow source")
	}
	seen := make(map[string]bool, len(input.References))
	for _, reference := range input.References {
		identity := reference.RepositoryIdentity
		if seen[identity] {
			continue
		}
		seen[identity] = true
		...
		`, identity).Scan(&repositoryID)
		if errors.Is(err, sql.ErrNoRows) {
			return conflict("repository_not_managed", fmt.Sprintf(
				"repository %s is not enabled in the managed repository catalog", identity,
			))
		}
		if err != nil {
			return unavailable(err)
		}
		if _, err := s.RefreshRepositoryWorkflowCatalog(ctx, repositoryID); err != nil {
			return err
		}
	}
	return nil
...
func resolveStandardBuildPrompt(target protocol.WorkTarget, workflow protocol.RepositoryWorkflowSnapshot) string {
	prompt := protocol.StandardBuildProcedurePrompt
	if workflow.ID != protocol.RepositoryWorkflowFallback || workflow.Instructions != protocol.StandardBuildProcedurePrompt {
