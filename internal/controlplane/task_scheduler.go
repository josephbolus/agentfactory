...
func (s *Store) AdmitDueTasks(ctx context.Context, limit int) error {
	...
	for index := 0; index < limit; index++ {
		...
		snapshot, err = s.freezePendingTaskWorkflows(ctx, id, due, snapshot)
		if err != nil {
			if finishErr := s.finishTaskOccurrence(ctx, id, due, false, err); finishErr != nil {
				return finishErr
			}
			continue
		}
...
func loadCurrentTaskSnapshot(ctx context.Context, tx *sql.Tx, id string) (protocol.TaskSnapshot, error) {
	...
	err := tx.QueryRowContext(ctx, `
		SELECT id, name, prompt, runtime, COALESCE(execution_profile_id, ''), timeout_seconds, concurrency_limit, generation,
		       outcome_contract, COALESCE(pipeline_id, ?), COALESCE(repository_workflow_id, ''), cron, timezone
		FROM tasks WHERE id = ?
	`, protocol.DefaultPipelineID, id).Scan(&snapshot.ID, &snapshot.Name, &snapshot.Prompt, &snapshot.Runtime,
		...
		&snapshot.OutcomeContract, &snapshot.Pipeline.ID, &snapshot.WorkflowID,
...
func (s *Store) freezePendingTaskWorkflows(
	ctx context.Context,
	id string,
	due time.Time,
	snapshot protocol.TaskSnapshot,
) (protocol.TaskSnapshot, error) {
	if snapshot.WorkflowID == "" || len(snapshot.RepositoryWorkflows) != 0 {
		return snapshot, nil
	}
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
	...
	result, err := tx.ExecContext(ctx, `
		UPDATE tasks SET pending_snapshot_json = ?
		WHERE id = ? AND pending_due_at = ?
	`, encoded, id, due.UnixMilli())
	if err != nil {
		return snapshot, unavailable(err)
	}
	if changed, _ := result.RowsAffected(); changed != 1 {
		return snapshot, conflict("task_occurrence_changed", "the pending Task occurrence changed")
	}
	if err := tx.Commit(); err != nil {
		return snapshot, unavailable(err)
	}
