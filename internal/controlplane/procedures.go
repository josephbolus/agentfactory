...
func (s *Store) AdmitProcedureRun(
	...
	if snapshot.WorkflowID != "" {
		workflow, err := loadWorkflowSnapshot(ctx, tx, snapshot.Repositories[0].ID, snapshot.WorkflowID)
		if err != nil {
			return protocol.ProcedureRunAdmission{}, err
		}
	...
	for position, target := range targets {
		...
		if snapshot.WorkflowID != "" {
			workflow, err = loadWorkflowSnapshot(ctx, tx, target.repository.ID, snapshot.WorkflowID)
			if err != nil {
				return protocol.ProcedureRunAdmission{}, err
			}
		}
		resolvedPrompt := resolveTaskWorkflowPrompt(snapshot.Prompt, workflow, snapshot.WorkflowID != "")
		...
		`, workID, runID, target.repository.ID, target.repository.RemoteIdentity, resolvedPrompt, snapshot.Runtime,
...
func loadProcedureSnapshot(ctx context.Context, tx *sql.Tx, nameKey string) (protocol.TaskSnapshot, error) {
	...
	err := tx.QueryRowContext(ctx, `
		SELECT id, name, prompt, runtime, COALESCE(execution_profile_id, ''),
			...
			COALESCE(pipeline_id, ?),
		       COALESCE(repository_workflow_id, ''),
	...
	`, protocol.DefaultPipelineID, nameKey).Scan(&snapshot.ID, &snapshot.Name, &snapshot.Prompt, &snapshot.Runtime,
		...
		&snapshot.Generation, &snapshot.OutcomeContract, &snapshot.Pipeline.ID, &snapshot.WorkflowID,
