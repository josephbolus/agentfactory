...
func TestTaskRunSnapshotsRepositoryWorkflow(t *testing.T) {
	store := newTestStore(t)
	...
	worker := registerTestWorker(t, store, workerA, 10, protocol.RepositoryRegistration{Key: "factory", RemoteIdentity: repository.RemoteIdentity})
	_ = worker
	...
	task, err := store.CreateTask(context.Background(), protocol.SaveTaskRequest{
		Name: "QA workflow", Prompt: "Run the selected checks.", Runtime: protocol.RuntimeCodex,
		WorkflowID: "qa", RepositoryIDs: []string{repository.ID},
	})
	if err != nil {
		t.Fatal(err)
	}
	if task.WorkflowID != "qa" {
		t.Fatalf("task workflow = %q", task.WorkflowID)
	}
	detail, _, err := store.RunTask(context.Background(), task.ID, protocol.RunTaskRequest{RequestKey: "qa-workflow-run"})
	if err != nil {
		t.Fatal(err)
	}
	if detail.Run.Task.WorkflowID != "qa" || detail.Sessions[0].Workflow.ID != "qa" ||
		!strings.Contains(detail.Sessions[0].ResolvedPrompt, "Run QA.") {
		t.Fatalf("workflow run = %#v", detail)
	}
}
