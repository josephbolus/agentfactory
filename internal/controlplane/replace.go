...
func (s *Store) ReplaceWork(
	...
	err = tx.QueryRowContext(ctx, `
		SELECT session.run_id, session.repository_id, session.repository_identity,
		       ...
		       session.source_kind, session.source_key, session.source_reference, session.source_title,
		       ...
		       session.repository_workflow_snapshot,
	...
	`, input.WorkID).Scan(
		...
		&predecessor.Target.SourceTitle, &predecessor.Target.ContextSnapshot, &oldPublishBranch,
		...
		&predecessor.Execution.CommitResolutionPolicy, &workflowSnapshot, &taskID, &taskSnapshotJSON,
		&outcomeContract,
	)
	if errors.Is(err, sql.ErrNoRows) {
	...
	target := protocol.WorkTarget{
		...
		SourceTitle: predecessor.Target.SourceTitle, ContextSnapshot: predecessor.Target.ContextSnapshot,
		PublishBranch: newPublishBranch,
	}
	...
	`, workID, runID, predecessor.RepositoryID, predecessor.RepositoryIdentity,
		...
		target.SourceKind, target.SourceKey, target.SourceReference, target.SourceTitle, target.ContextSnapshot,
		target.PublishBranch, input.WorkID,
		boundedUTF8Bytes(blockedReason, protocol.MaxWaitingReasonBytes), workflowSnapshot); err != nil {
		return protocol.WorkReplacement{}, unavailable(err)
	}
