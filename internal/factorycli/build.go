...
func (c command) runBuild(server string, jsonOutput bool, arguments []string) error {
	...
	workflow := flags.String("workflow", "", "select a repository workflow by ID")
	...
	if explicit["workflow"] && strings.TrimSpace(*workflow) == "" {
		return &usageError{message: "--workflow requires a non-empty value"}
	}
	...
	input := protocol.BuildRequest{
		...
		Workflow: strings.TrimSpace(*workflow), WorkflowSpecified: explicit["workflow"],
		Rebuild: *rebuild,
	}
...
func interspersedBuildArguments(arguments []string) ([]string, error) {
	...
	for index := 0; index < len(arguments); index++ {
		...
		switch name {
		case "repo", "runtime", "workflow", "request-key":
