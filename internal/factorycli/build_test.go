...
func TestBuildWorkflowFlagIsSentAsExplicitSelection(t *testing.T) {
	requests := make(chan protocol.BuildRequest, 1)
	server := httptest.NewServer(http.HandlerFunc(func(output http.ResponseWriter, request *http.Request) {
		var input protocol.BuildRequest
		if err := json.NewDecoder(request.Body).Decode(&input); err != nil {
			t.Error(err)
	...
	t.Cleanup(server.Close)
	var stdout, stderr bytes.Buffer
	if code := Run(Options{Arguments: []string{
		"--server", server.URL, "build", "--repo", "github.com/acme/api",
		"--workflow", "dba/index-review", "ISSUE-1",
	}, Stdout: &stdout, Stderr: &stderr}); code != 0 {
		t.Fatalf("workflow Build = code %d, stdout %q, stderr %q", code, stdout.String(), stderr.String())
	}
	input := <-requests
	if input.Workflow != "dba/index-review" || !input.WorkflowSpecified {
		t.Fatalf("workflow request = %#v", input)
	}
}
