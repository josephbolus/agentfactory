...
func TestRuntimeEnvironmentPrependsSiblingFactoryDirectory(t *testing.T) {
	t.Setenv("PATH", "/usr/bin")
	environment := runtimeEnvironment("", "", "", "", "", "/private/factory/bin/factory")
	if !containsEnvironment(environment, "PATH=/private/factory/bin:/usr/bin") {
		t.Fatalf("environment does not inject the Factory CLI directory: %v", environment)
	}
}

func TestSiblingFactoryExecutable(t *testing.T) {
	directory := t.TempDir()
	workerPath := filepath.Join(directory, "factory-worker")
	factoryPath := filepath.Join(directory, "factory")
	if err := os.WriteFile(factoryPath, []byte("#!/bin/sh\n"), 0o700); err != nil {
		t.Fatal(err)
	}
	if got := siblingFactoryExecutable([]string{workerPath, supervisorCommand}); got != factoryPath {
		t.Fatalf("siblingFactoryExecutable() = %q, want %q", got, factoryPath)
	}
	if got := siblingFactoryExecutable([]string{"factory-worker", supervisorCommand}); got != "" {
		t.Fatalf("relative supervisor command resolved %q", got)
	}
}
