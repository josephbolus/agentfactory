...
func checkHealth(
	...
	requiredProfiles []ProfileConfig,
	...
	for index, runtime := range runtimes {
		...
		probes = append(probes, func() {
			runtimeCapabilities[index] = runtimeCapabilityWithin(
				ctx, runtime, runtimeExecutables[runtime], healthCheckTimeout,
				profilesForAdapter(requiredProfiles, runtime)...,
			)
		})
	}
	...
	rolesReady := readyRuntime
	if len(requiredRuntimes) != 0 {
		rolesReady = requiredRuntimesReady(result.Capabilities, requiredRuntimes)
	}
	if gitCapability.Status == protocol.CapabilityReady && rolesReady {
	...
	if gitCapability.Status != protocol.CapabilityReady {
		...
		result.Error = errors.New("a configured role runtime is not ready")
	}
	return result
}

func requiredRuntimesReady(capabilities []protocol.Capability, runtimes []string) bool {
	for _, runtime := range runtimes {
		if !capabilityReady(capabilities, protocol.CapabilityKindRuntime, runtime) {
			return false
		}
	}
	return true
...
func runtimeCapabilityWithin(
	...
	case protocol.RuntimePi:
		...
		for _, profile := range profiles {
			if profile.Adapter != protocol.RuntimePi || piModelAvailable(models, profile.Provider, profile.Model) {
				continue
			}
			capability.Status = protocol.CapabilityUnauthenticated
			capability.Message = fmt.Sprintf(
				"Pi model %s/%s is unavailable or unauthenticated.",
...
func parsePiModels(output []byte) []piModel {
	lines := strings.Split(strings.TrimSpace(string(output)), "\n")
	models := make([]piModel, 0, len(lines))
	for _, line := range lines {
		fields := strings.Fields(line)
		if len(fields) < 6 || fields[0] == "provider" && fields[1] == "model" {
			continue
		}
...
func piModelAvailable(models []piModel, provider, model string) bool {
	for _, candidate := range models {
		if strings.EqualFold(candidate.provider, provider) && candidate.model == model {
			return true
		}
	}
	return false
...
func profilesForAdapter(profiles []ProfileConfig, adapter string) []ProfileConfig {
	result := make([]ProfileConfig, 0, len(profiles))
	for _, profile := range profiles {
