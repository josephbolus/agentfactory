...
type supervisorInit struct {
	...
	FactoryExecutable string `json:"factory_executable,omitempty"`
}
...
func superviseRuntime(
	...
	arguments, err := runtimeArguments(init.Runtime, init.Provider, init.Model, init.ReasoningEffort)
	if err != nil {
	...
	command.Env = runtimeEnvironment(
		init.RunID, init.SessionID, init.AttemptID, init.UpdateSocket, init.UpdateToken, init.FactoryExecutable,
	)
...
func runtimeArguments(runtime, provider, model, effort string) ([]string, error) {
	if provider == "" && model == "" && effort == "" {
		switch runtime {
		case protocol.RuntimePi:
			return []string{"--print", "--no-session"}, nil
		case protocol.RuntimeCodex:
			return []string{"exec", "--json", "--color", "never"}, nil
		case protocol.RuntimeClaudeCode:
			return []string{"--print", "--output-format", "stream-json", "--verbose", "--permission-mode", "bypassPermissions"}, nil
		}
	}
	if provider == "" || model == "" || !supportedEffort(effort) {
		return nil, errors.New("runtime profile is incomplete")
	}
	switch runtime {
	case protocol.RuntimePi:
		return []string{"--print", "--no-session", "--provider", provider, "--model", model, "--thinking", effort}, nil
	case protocol.RuntimeCodex:
		return []string{"exec", "--json", "--color", "never", "--model", model, "-c", "model_reasoning_effort=" + effort}, nil
	case protocol.RuntimeClaudeCode:
		return []string{"--print", "--output-format", "stream-json", "--verbose", "--permission-mode", "bypassPermissions", "--model", model, "--effort", effort}, nil
	default:
		return nil, errors.New("unsupported worker runtime")
	}
}

func readOnlyArguments(profile ProfileConfig) ([]string, error) {
	...
	case protocol.RuntimeCodex:
		return append(arguments, "--sandbox", "read-only"), nil
	case protocol.RuntimeClaudeCode:
		for index, value := range arguments {
			if value == "bypassPermissions" {
				arguments[index] = "plan"
			}
		}
		return arguments, nil
	case protocol.RuntimePi:
		return append(arguments, "--tools", "read,grep,find,ls"), nil
	default:
		return nil, errors.New("unsupported worker runtime")
	}
}

func structuredOutputArguments(profile ProfileConfig) ([]string, error) {
	arguments, err := readOnlyArguments(profile)
	if err != nil || profile.Adapter != protocol.RuntimeClaudeCode {
	...
	return nil, errors.New("Claude arguments require JSON output")
}
...
func runtimeEnvironment(runID, sessionID, attemptID, updateSocket, updateToken, factoryExecutable string) []string {
	...
	if factoryExecutable != "" {
		factoryDirectory := filepath.Dir(factoryExecutable)
		for index, value := range environment {
			if path, found := strings.CutPrefix(value, "PATH="); found {
				environment[index] = "PATH=" + factoryDirectory + string(os.PathListSeparator) + path
				return environment
			}
		}
		environment = append(environment, "PATH="+factoryDirectory)
	}
	return environment
}

func siblingFactoryExecutable(supervisorCommand []string) string {
	if len(supervisorCommand) == 0 || !filepath.IsAbs(supervisorCommand[0]) {
		return ""
	}
	candidate := filepath.Join(filepath.Dir(supervisorCommand[0]), "factory")
