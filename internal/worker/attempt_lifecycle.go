...
func (manager *Manager) runAttempt(parent context.Context, claim protocol.Claim, token string) {
	...
	if err := manager.commentProgress(handle.context, claim, value,
		"Planning: running.\n\nImplementation: pending.\n\nReview: pending."); err != nil {
		manager.finishWithWorktree(claim, token, handle, repository, value, "failed", "", err.Error())
		return
	}
	executor, err := manager.executorProfile(claim.Execution.RequiredRuntime)
	if err != nil {
		commentErr := manager.commentProgress(handle.context, claim, value,
			"Planning: blocked.\n\nImplementation: blocked.\n\nReview: skipped.\n\nError: "+err.Error())
		if commentErr != nil {
			err = errors.Join(err, commentErr)
		}
		manager.finishWithWorktree(claim, token, handle, repository, value, "failed", "", err.Error())
		return
	}
	planPath, err := manager.planWork(handle.context, claim, value)
	if err != nil {
		commentErr := manager.commentProgress(handle.context, claim, value,
			"Planning: failed.\n\nImplementation: blocked.\n\nReview: skipped.\n\nError: "+err.Error())
		if commentErr != nil {
	...
	if planPath != "" {
		planning = "Planning: completed (`" + planPath + "`)."
		if err := manager.commentProgress(handle.context, claim, value,
			planning+"\n\nImplementation: running.\n\nReview: pending."); err != nil {
			manager.finishWithWorktree(claim, token, handle, repository, value, "failed", "", err.Error())
			return
		}
	} else if err := manager.commentProgress(handle.context, claim, value,
		planning+"\n\nImplementation: running.\n\nReview: pending."); err != nil {
		manager.finishWithWorktree(claim, token, handle, repository, value, "failed", "", err.Error())
		return
	}
	...
	finishExecution := func(state, result, failure string) {
		if commentErr := manager.commentProgress(handle.context, claim, value, executionStage(planning, state, failure)); commentErr != nil {
			if failure == "" {
				failure = commentErr.Error()
			} else {
				failure = errors.Join(errors.New(failure), commentErr).Error()
			}
			state = "failed"
		}
		manager.finishWithWorktree(claim, token, handle, repository, value, state, result, failure)
	}
	for index, stage := range stages {
		...
		if reason := handle.stopReasonAt(time.Now()); reason != "" {
			...
			finishExecution(terminalForStop(handle), "", err.Error())
			return
		}
		...
		if planPath != "" {
			prompt += "\n\nFactory plan artifact: " + planPath + "\nRead and follow it before implementation."
		}
		if len([]byte(value.Branch)) > protocol.MaxAgentBranchBytes ||
			...
			finishExecution("failed", "", err.Error())
			return
		}
		...
		process, err := startSupervisor(manager.options.SupervisorCommand, supervisorInit{
			Runtime: executor.Adapter, RuntimeExecutable: manager.runtimeExecutable(executor.Adapter),
			Provider: executor.Provider, Model: executor.Model, ReasoningEffort: executor.ReasoningEffort,
			...
			FactoryExecutable: siblingFactoryExecutable(manager.options.SupervisorCommand),
		...
		if err := process.awaitReady(handle.context); err != nil {
			...
			sender.closeAndWait(5 * time.Second)
			finishExecution(terminalForStop(handle), "", stoppedAttemptError(handle, err).Error())
			return
		}
		...
		if firstExecutedStage {
			...
			if err != nil {
				...
				finishExecution(terminalState(message), message.Result, firstNonEmpty(errorText, message.Error))
				return
			}
			if started.State != "running" {
				...
				message := manager.waitForSupervisor(process)
				sender.closeAndWait(5 * time.Second)
				finishExecution("failed", message.Result, "control plane did not accept the running transition")
				return
			}
		}
		...
		if err != nil {
			...
			message := manager.waitForSupervisor(process)
			sender.closeAndWait(5 * time.Second)
			finishExecution(terminalState(message), message.Result, err.Error())
			return
		}
		...
		if firstExecutedStage {
			if _, err := manager.client.start(handle.context, claim.Attempt.ID, protocol.StartAttemptRequest{
				...
				message := manager.waitForSupervisor(process)
				sender.closeAndWait(5 * time.Second)
				finishExecution(terminalState(message), message.Result, firstNonEmpty(err.Error(), message.Error))
				return
			}
		...
		if completeErr != nil || stageState != protocol.StageSucceeded {
			...
			failure := firstNonEmpty(errorString(completeErr), message.Error)
			finishExecution(attemptState, message.Result, failure)
			return
		}
	...
	reviewed := false
	if manager.config.Roles.Reviewer != "" && terminalState(finalMessage) == "succeeded" {
		if err := manager.commentProgress(handle.context, claim, value,
			reviewProgressStage(planning)); err != nil {
			manager.finishWithWorktree(claim, token, handle, repository, value, "failed", lastResult, err.Error())
			return
		}
		reports, repaired, err := manager.reviewWork(handle.context, claim, repository, value)
		if err != nil {
			commentErr := manager.commentReviewFailure(handle.context, claim, value, reports, repaired, err)
			if commentErr != nil {
				err = errors.Join(err, commentErr)
			}
			manager.finishWithWorktree(claim, token, handle, repository, value, "failed", lastResult, err.Error())
		...
		if err := manager.commentReview(handle.context, claim, value, reports, repaired); err != nil {
			manager.finishWithWorktree(claim, token, handle, repository, value, "failed", lastResult, err.Error())
			return
		}
		if reports[len(reports)-1].Verdict != reviewApproved {
			manager.finishWithWorktree(claim, token, handle, repository, value, "failed", lastResult,
				"independent review still has findings")
			return
		}
		reviewed = true
	}
	...
	finishExecution(terminalState(finalMessage), lastResult, finalMessage.Error)
}
