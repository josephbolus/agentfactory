...
const githubIssuePath = /\/issues\/([1-9]\d*)$/;

function workTitle(run: Run): string {
  if (run.targets?.length !== 1) {
    return run.task.name;
  }
  const target = run.targets[0];
  if (target.source_kind !== "github_issue" || !target.source_title) {
    return run.task.name;
  }
  const match = target.source_reference.match(githubIssuePath);
  if (!match) {
    return run.task.name;
  }
  return `#${match[1]} ${target.source_title}`;
}
...
function RunBoardCard({ run, onClick }: { run: Run; onClick: () => void }) {
  ...
  return <button className={`work-card work-card-${run.state}`} onClick={onClick} aria-label={`${title}, ${run.state}, ${run.session_count} sessions`}>
    ...
    <span className="work-card-tags">
      ...
      {title !== run.task.name && <span className="work-chip">{run.task.name}</span>}
      ...
      {run.task.workflow_id && <span className="work-chip">{run.task.workflow_id}</span>}
    </span>
    <Progress run={run} />
...
export function RunDetailView({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  return <div className="page run-detail-clean">
    ...
    <section className="run-summary-strip"><div><span>Procedure</span><strong>{run.task.name}</strong></div><div><span>Workflow</span><strong>{run.task.workflow_id ?? "implement"}</strong></div><div><span>Pipeline</span><strong>{run.task.pipeline?.name ?? "Single agent"}</strong></div><div><span>Stages</span><strong>{run.task.pipeline?.stages.length ?? 1}</strong></div><div><span>Completed</span><strong>{successfulSessions(run)}</strong></div><div><span>Duration</span><strong>{run.terminal_at ? duration(run.admitted_at, run.terminal_at) : "Active"}</strong></div></section>
