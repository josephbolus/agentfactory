...
function profileDescription(profile: ExecutionProfile | undefined): string {
  if (!profile || profile.id === persistentAutoProfileID) return "Agent Factory selects the least-loaded eligible local or VM Worker.";
      ...
      <div className="task-list panel">
        {query.data.map((task) => <article className="task-row" key={task.id}>
          <button className="task-copy" onClick={() => openTask(task.id)}>
            ...
            <small><GitBranch size={12} /> {task.repository_count} repos · {task.pipeline_name ?? "Single agent"} · {task.workflow_id ?? "implement"} · {task.runtime}</small>
          </button>
...
function TaskComposer({ task, onClose, onSaved, onArchive, archiveError, archivePending }: { task?: Task; onClose: () => void; onSaved: () => void; onArchive: (task: Task) => void; archiveError: Error | null; archivePending: boolean }) {
  ...
  const [prompt, setPrompt] = useState(task?.prompt ?? "");
	...
	const [profileID, setProfileID] = useState(task?.execution_profile_id ?? persistentAutoProfileID);
	const [workflowID, setWorkflowID] = useState(task?.workflow_id ?? "implement");
  ...
  const [cron, setCron] = useState(task?.schedule.cron ?? "0 9 * * 1");
	...
	const workflows = useQuery({
		queryKey: ["repository-workflow-options", selected],
		queryFn: () => api.workflowOptions(selected),
		enabled: selected.length > 0,
	});
	const workflowAvailable = workflows.data?.some((workflow) => workflow.id === workflowID) ?? false;
	const workflowFallback = workflows.data?.some((workflow) => workflow.id === "implement") ? "implement" : "";
	const selectedWorkflowID = workflows.isSuccess && !workflowAvailable ? workflowFallback : workflowID;
  ...
  const selectedPipeline = pipelines.data?.find((pipeline) => pipeline.id === pipelineID);
	...
	const workflowMissing = selected.length > 0 && workflows.isSuccess && !workflowAvailable && workflowFallback === "";
	const workflowUnavailable = selected.length > 0 && (workflows.isPending || workflows.isError || !workflows.data || workflowMissing);
    ...
    <section className="modal task-modal" role="dialog" aria-modal="true" aria-labelledby="task-composer-title">
      ...
      <div className="modal-body task-form">
        ...
        <label className="field"><span>Prompt</span><textarea className="task-prompt" disabled={readOnly} value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Review the repository and fix..." /></label>
	        ...
	        <label className="field"><span>Workflow</span><select aria-label="Workflow" disabled={readOnly || workflowUnavailable} value={selectedWorkflowID} onChange={(event) => setWorkflowID(event.target.value)}>
	          {selectedWorkflowID === "" && <option value="">No compatible workflow</option>}
	          {workflows.isPending && <option value={workflowID}>Loading repository workflows…</option>}
	          {(workflows.data ?? []).map((workflow) => <option key={workflow.id} value={workflow.id}>{workflow.title} · {workflow.id}</option>)}
	          {!workflows.isPending && workflows.isError && <option value={workflowID}>Workflow catalog unavailable</option>}
	        </select><small className={workflows.isError || workflowMissing ? "field-error" : "field-hint"}>{workflows.isError ? "Refresh repository workflows before saving." : workflowMissing ? "Selected workflow is unavailable for the selected repositories." : "Select the repository instructions applied to each run."}</small></label>
