import type {
  ...
  SaveTaskInput,
  Pipeline,
  RepositoryWorkflowCatalog,
  RepositoryWorkflowOption,
  SavePipelineInput,
  RunDetail,
  RunPage,
  Worker,
...
export const api = {
  ...
  setRepositoryIssueIntake: (id: string, enabled: boolean) => request<ManagedRepository>(`/api/v1/repositories/${encodeURIComponent(id)}/issue-intake`, {
    method: "PUT", body: JSON.stringify({ enabled }),
  }),
  repositoryWorkflows: (id: string) => request<RepositoryWorkflowCatalog>(`/api/v1/repositories/${encodeURIComponent(id)}/workflows`),
  refreshRepositoryWorkflows: (id: string) => request<RepositoryWorkflowCatalog>(`/api/v1/repositories/${encodeURIComponent(id)}/workflows/refresh`, {
    method: "POST", body: "{}",
  }),
  workflowOptions: (repositoryIDs: string[]) => request<{ workflows: RepositoryWorkflowOption[] }>("/api/v1/repository-workflows/options", {
