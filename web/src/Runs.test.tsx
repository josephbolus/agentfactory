...
describe("Runs", () => {
  ...
  it("shows a GitHub issue title and keeps its Procedure visible", async () => {
    const issue = issueRun();
    vi.spyOn(api, "runs").mockResolvedValue({ runs: [issue], next_cursor: null });
    const client = testClient();

    render(<QueryClientProvider client={client}><RunsView mode="kanban" onMode={() => undefined} onRun={() => undefined} /></QueryClientProvider>);

    const card = await screen.findByRole("button", { name: /#42 Fix case-insensitive search, running/ });
    expect(card).toHaveTextContent("standard-build");
  });

  it("shows the GitHub issue title and Procedure in Run detail", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "#42 Fix case-insensitive search" })).toBeVisible();
    expect(screen.getByText("Procedure").parentElement).toHaveTextContent("Procedurestandard-build");
  });
...
function issueRun(): Run {
  return {
    ...run("run-issue", "standard-build", "running"),
    targets: [{
      id: "work-1", position: 0, target_key: "github:example:factory:issue:42",
      target_kind: "work_item", repository_id: "repository-1",
      repository_identity: "github.com/example/factory", source_kind: "github_issue",
      source_key: "github:example:factory:issue:42",
      source_reference: "https://github.com/example/factory/issues/42",
      source_title: "Fix case-insensitive search", publish_branch: "factory/work-1",
    }],
  };
}
