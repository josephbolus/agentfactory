...
const managedTaskName = "E2E managed repository review";
...
test("creates a Task and completes its Run", async ({ page }) => {
  ...
  await expect(dialog.getByLabel("Workflow")).toBeEnabled();
  await expect(dialog.getByLabel("Workflow")).toHaveValue("implement");
...
test("acquires a centrally managed repository", async ({ page }) => {
  await page.goto("/tasks");
  await page.getByRole("button", { name: "New Task", exact: true }).first().click();
  const dialog = page.getByRole("dialog", { name: "New Task" });
  await dialog.getByLabel("Name").fill(managedTaskName);
  await dialog.getByLabel("Prompt").fill("Review the dynamically acquired repository.");
  await dialog.getByLabel("Pipeline").selectOption({ label: `${pipelineName} · 3 stages` });
  await dialog.getByRole("button", { name: "github.com/example/managed-demo" }).click();
  await dialog.getByRole("button", { name: "Save Task" }).click();

  const task = page.locator("article").filter({ hasText: managedTaskName });
  await task.getByRole("button", { name: "Run now" }).click();
  const runDialog = page.getByRole("dialog", { name: `Run ${managedTaskName}` });
  await runDialog.getByRole("button", { name: "Run now" }).click();
