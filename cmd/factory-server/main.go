...
func run() (returnErr error) {
	const defaultGitHubIssuePollInterval = 30 * time.Second
	...
	githubIssuePollInterval := defaultGitHubIssuePollInterval
	if bootstrap.GitHubIssuePollInterval != "" {
		githubIssuePollInterval, err = time.ParseDuration(bootstrap.GitHubIssuePollInterval)
		if err != nil || githubIssuePollInterval <= 0 {
			return errors.New("github_issue_poll_interval must be a positive duration")
		}
	}
	...
	githubIntakeContext, cancelGitHubIntake := context.WithCancel(rootContext)
	githubIntakeDone := make(chan struct{})
	go func() {
		defer close(githubIntakeDone)
		store.RunGitHubIssueIntake(githubIntakeContext, logger, githubIssuePollInterval)
	}()
	defer func() {
		cancelGitHubIntake()
		<-githubIntakeDone
