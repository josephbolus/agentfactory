...
- When running `gh` commands in a sandbox, use `require_escalated` because
  `gh` may need access to credentials stored in the host keychain.
...
- Stage only task files. A dirty worktree does not justify omitting a commit.

- Before reporting publication, verify the exact remote ref with `git ls-remote
  origin main`.
