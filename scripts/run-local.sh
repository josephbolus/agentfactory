...
if [ -z "$data_home" ]; then
  if [ -z "${HOME:-}" ]; then
    echo "Agent Factory startup requires HOME or FACTORY_DATA_HOME." >&2
    exit 1
  fi
  legacy_data_home=
  legacy_config=
  ...
  if [ -n "$legacy_data_home" ]; then
    echo "Agent Factory found preview state and refused to replace it with an empty ~/.factory home." >&2
...
case "$(uname -s)" in
  ...
  *)
    echo "Agent Factory local workers are supported only on Unix." >&2
    exit 1
    ;;
esac

if [ ! -f "$config" ]; then
  echo "Agent Factory worker configuration does not exist: $config" >&2
  echo "Copy examples/worker.toml there and set the repository paths first." >&2
  exit 1
fi

if ! command -v curl >/dev/null 2>&1; then
  echo "Agent Factory local startup requires curl on PATH." >&2
  exit 1
fi
...
echo "Starting Agent Factory server on http://$listen/ ..."
if [ -n "$listen_override" ]; then
...
while [ "$(date +%s)" -lt "$server_ready_deadline" ]; do
  if ! kill -0 "$server_pid" 2>/dev/null; then
    ...
    echo "Agent Factory server exited before becoming ready. Check the error above." >&2
    exit 1
  fi
...
if [ "$ready" != "1" ]; then
  echo "Agent Factory server did not become healthy within $server_ready_seconds seconds." >&2
  exit 1
fi

echo "Starting Agent Factory worker with $config ..."
...
while [ "$(date +%s)" -lt "$worker_ready_deadline" ]; do
  if ! kill -0 "$server_pid" 2>/dev/null; then
    wait "$server_pid" || true
    echo "Agent Factory server stopped during worker startup." >&2
    exit 1
  fi
  if ! kill -0 "$worker_pid" 2>/dev/null; then
    wait "$worker_pid" || true
    echo "Agent Factory worker exited during startup. Check the configuration and error above." >&2
    exit 1
  fi
...
if [ "$registered" != "1" ]; then
  echo "Agent Factory worker did not register as healthy within $worker_ready_seconds seconds. Check its health errors above." >&2
  exit 1
fi
if ! kill -0 "$server_pid" 2>/dev/null; then
  wait "$server_pid" || true
  echo "Agent Factory server stopped before startup completed." >&2
  exit 1
fi
if ! kill -0 "$worker_pid" 2>/dev/null; then
  wait "$worker_pid" || true
  echo "Agent Factory worker stopped before startup completed." >&2
  exit 1
fi

echo "Agent Factory is ready at http://$listen/"
echo "Press Ctrl-C to stop the server and worker. State remains in $data_home."

while :; do
  if ! kill -0 "$server_pid" 2>/dev/null; then
    wait "$server_pid" || true
    echo "Agent Factory server stopped unexpectedly." >&2
    exit 1
  fi
  if ! kill -0 "$worker_pid" 2>/dev/null; then
    wait "$worker_pid" || true
    echo "Agent Factory worker stopped unexpectedly." >&2
    exit 1
  fi
  sleep 1
done
