...
Agent Factory is a local-first control plane for repeatable software-engineering
...
artifacts, and credentials are not implemented yet. The contract keeps Agent Factory
...
Managed repositories may version Markdown workflows under
`.factory/workflows/`, including nested directories. Each file has YAML
frontmatter with a stable lowercase slash-separated `id`, `title`, optional
`description`, and optional `github_issue.labels_all`. The Markdown body is
trusted repository policy; GitHub issue title, body, labels, comments, and
linked content remain untrusted context.

The control plane reads the default-branch tree and blobs through the GitHub
CLI, parses and bounds every workflow, and persists one catalog per managed
repository. `implement.md` is the compatibility fallback when it has no
frontmatter. A GitHub intake issue matches one workflow only when all declared
labels are present (case-insensitive); zero matches use `implement`, while
ambiguous, invalid, or unavailable catalogs are blocked and recorded as
diagnostics. An explicit `factory build --workflow ID` refreshes each target
catalog before admission and rejects a workflow absent from any target.

Admission freezes workflow ID, path, commit SHA, digest, metadata, and the
Markdown body in the Run Task snapshot and each repository Work snapshot.
Later repository edits do not change queued or active Work. The browser lists
catalog metadata and diagnostics but never returns workflow instructions from
the catalog endpoint. GitHub intake posts or updates one marked Issue comment
with the selected workflow and Work ID after successful admission.
...
Agent Factory is a trusted single-operator system. It has no multi-user tenant model.
...
| Repository workflow routing | `internal/protocol/repository_workflows.go`, `internal/controlplane/repository_workflows.go`, `internal/controlplane/github_intake.go`, `internal/controlplane/repository_workflows_http.go`, `migrations/038_repository_workflow_catalogs.sql` |
...
| Schema | `migrations/027_routines_work.sql`, `migrations/030_task_run_session.sql`, `migrations/031_work_lifecycle.sql`, `migrations/032_agent_update_requests.sql`, `migrations/033_pipeline_templates.sql`, `migrations/034_resume_recovery.sql`, `migrations/038_repository_workflow_catalogs.sql` |
